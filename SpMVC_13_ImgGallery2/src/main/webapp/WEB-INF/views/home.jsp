...
<script>
	$(function() {
		...
		/*
		 jquery 응용. 마우스를 제어해서 context menu(마우스 우클릭 메뉴)를 만들어주는 툴
		 contextMenu.js랑 css, ui.js를 import
		 */
		$.contextMenu({
			selector : '.img_card', // 어떤 태그에서 작동할 것인가를 지정
			...
			if (key == 'delete') {
				if (confirm(id + "이미지를 삭제할까요")) {
					$.post("${rootPath}/rest/image_delete", {
						img_id : id
					}, function(result) { // 타입명시 안하고 쓸 수 있는 js
						if (result == "OK")
							// document.location.replace("${rootPath}/")
							document.location.replace(document.location.href)
						else {
							alert("파일을 삭제할 수 없음")
							document.location.replace(document.location.href) // 현재 화면 새로고침
						}
					})
				}
			} else if (key == "copy") {
		...
		$.contextMenu({
			selector : '.img_view', // 어떤 태그에서 작동할 것인가를 지정
