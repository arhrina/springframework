...
public class DeptVO {
	...
	@Size(min=5, max=5)
	@NotEmpty(message = "* 거래처코드는 반드시 입력되어야한다")
	private String d_code;
	
	@Column(length = 30, nullable = false)
	@NotEmpty(message = "* 거래처이름은 반드시 입력되어야한다")
	...
	@Size(min=13, max=13)
	@NotEmpty(message = "* 사업자번호는 반드시 입력되어야한다")
	private String d_sid; // 409-01-123456 형식
	
	@Column(length = 20)
	private String d_tel;
	
	// length가 없으면 기본값은 255
