...
public class ProductSerivce {
	...
	// sub file 삭제
	// tbl_files에서 file_seq에 해당하는 img file 정보를 가져온다 
	// 이미지 파일을 삭제하고 이미지 파일이 삭제가 완료되면 table에서 파일 정보를 제거
	public String subImgDelete(String file_seq) {
		// TODO Auto-generated method stub
		ProFileDTO profileDTO = fileDao.findByFileSeq(file_seq); // tbl_files에서 파일정보 가져오기
		String file_name = profileDTO.getFile_upload_name(); // 파일이름
		String p_code = profileDTO.getFile_p_code(); // 상품코드
		
		fService.fileDelete(file_name); // 이미지파일 삭제
	...
	// 상품코드를 매개변수로 받아서 대표이미지와 이미지정보를 제거
