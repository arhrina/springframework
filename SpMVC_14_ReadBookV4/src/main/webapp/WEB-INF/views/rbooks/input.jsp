...
$(function() {
		...
		$("#rb_bname, #rb_rtime, #rb_bcode, #rb_subject").focus(function(){ // 내용이 있을 때 클릭되면(focus가 위치하면)
			$(this).select() // 전체블록 선택되어 새로운 단어를 입력하면 삭제되도록
		})
	
		$("#rb_bname").keypress(function(event) { // 책이름에서 엔터키를 누르면 function 동작
			if(event.keyCode == 13) { // 엔터키는 13
				let strExt = $(this).val() // 변수 strExt에 값을 담는다
				if(strExt == "") {
		...
		<article>
			<form:form modelAttribute="rBookVO"> <!-- spring form태그는 생략해도 method="POST"이 입력된 상태 -->
				<div class="input-box">
					...
					<form:input path="rb_bname" placeholder="도서이름을 입력한 후 엔터"/>
				</div>
				<div class="input-box">
