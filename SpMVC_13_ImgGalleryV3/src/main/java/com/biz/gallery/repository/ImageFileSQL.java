...
public class ImageFileSQL {
	...
	// 메서드가 아니라 변수다
	public final static String bulk_insert_sql = "<script>" // 문자열을 xml코드로 만듦
			+ "INSERT INTO tbl_images (" + "		IMG_FILE_SEQ," + "		IMG_FILE_P_CODE,"
			+ "		IMG_FILE_ORIGIN_NAME," + "		IMG_FILE_UPLOAD_NAME" + ") SELECT SEQ_IMAGES.NEXTVAL, SUB.*  FROM ("
			+ "<foreach collection='files' item='file' separator='UNION ALL'>" + "	SELECT "
			+ "			#{img_p_code,jdbcType=VARCHAR},"
			// +" #{file.img_file_p_code,jdbcType=VARCHAR},"
			+ "			#{file.img_file_origin_name,jdbcType=VARCHAR},"
			+ "			#{file.img_file_upload_name,jdbcType=VARCHAR}" + "	FROM DUAL" + "</foreach>" + ") SUB"
			+ "</script>";
