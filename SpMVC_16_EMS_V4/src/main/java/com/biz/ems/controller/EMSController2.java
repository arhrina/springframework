...
//		xMailService.sendMail(emailVO);
	...
	@RequestMapping(value="/view/{ems}", method=RequestMethod.GET) // 뒤에 중괄호 붙는 이름은 pathVariable 이름과 같으면 된다
	public String view(@ModelAttribute("emailVO") EmailVO emailVO, @PathVariable("ems") String ems_seq, Model model) {
	...
	@RequestMapping(value="/update/{ems}", method=RequestMethod.GET) // 뒤에 중괄호 붙는 이름은 pathVariable 이름과 같으면 된다
	public String update(@ModelAttribute("emailVO") EmailVO emailVO, @PathVariable("ems") String ems_seq, Model model) {
		emailVO = mailService.findBySeq(Long.valueOf(ems_seq));
		model.addAttribute("BODY", "WRITE");
		model.addAttribute("emailVO", emailVO);
		return "home";
