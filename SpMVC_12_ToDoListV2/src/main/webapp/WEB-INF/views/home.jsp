...
<body>
	...
	<section>
		<article id="input">
			<form method="POST">
				<input type="hidden" name="td_complete"
					value='${todoDTO.td_complete}'>
				<input type="hidden" name="td_seq" value='<c:out value="${todoDTO.td_complete}" default="0"/>'>
				...
				<table class="input">
					<tr>
						...
						<td><input type="text" value="${todoDTO.td_subject}"
							name="td_subject"></td>

						<td><input type="checkbox" id="tdAlaram" name="td_alarm"
							<c:if test="${todoDTO.td_alarm == 'Y'}" >
							checked="checked"
							</c:if>
							value="Y"> <label for="tdAlaram">알람설정</label></td>
				...
				<table class="list">
					...
					<c:forEach items="${todoList}" var="todo" varStatus="index">
						<tr>
							...
							<td><a
								href="${rootPath}/complete?tdSeq=${todo.td_seq}&tdComplete=${todo.td_complete}">
									${todo.td_complete} </a></td>
							<td><a
								href="${rootPath}/alarm?tdSeq=${todo.td_seq}&tdAlarm=${todo.td_alarm}">
									${todo.td_alarm} </td>
